...
public class IBeacon implements Serializable {
	 ...
	 * @param major
	 *            The Major number (1-65535)
	 * @param minor
	 *            The Minor number (1-65535)
	 * @param proximity
	 *            Proximity in meters
	 */
	...
	public String getUuidHexStringDashed() {
		...
		String newUuid = uuid.substring(0, 8) + "-" + uuid.substring(8, 12)
	...
	/**
	 * Two iBeacons are the same if UUID, major, minor and MAC addresses are the
	 * same. The MAC address comparison is to avoid detecting as the same two
	 * different iBeacons with factory settings.
	 */
	@Override
	public boolean equals(Object obj) {
		...
		if (this.isSameRegionAs(ibeacon)
				&& _macAddress.equals(ibeacon.getMacAddress()))
			return true;
		return false;
	}

	/**
	 * Returns true if the iBeacon to compare represents the same region (same
	...
	public boolean isSameRegionAs(IBeacon ibeacon) {
		...
		if (getUuidHexString().equals(ibeacon.getUuidHexString())
				&& _major == ibeacon.getMajor() && _minor == ibeacon.getMinor())
			return true;
		return false;
	}
	...
	public String toString() {
		return "UUID:" + this.getUuidHexString() + " M:" + this.getMajor()
				+ " m:" + this.getMinor() + " p:" + this.getProximity();
	}
}
