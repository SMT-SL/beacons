package com.smt.beacons.easibeacons;
...
public class IBeaconProtocol {
	 ...
	 */
	public static int SCANNING_PERIOD = 12000;
	/**
	...
	/**
	 * State of the search process to notify the listener: at least one iBeacon
	 * found
	 */
	...
	/**
	 * Reference to the previous nearest iBeacon, to identify if region has
	 * changed
	 */
	...
	/**
	 * Configures the listener that will receive events involving iBeacon
	 * regions
	 * 
	 * @param l
	 *            the listener to configure
	 */
	...
	/**
	 * Removes information about previous nearest beacon in order to "forget"
	 * the current region and detect it again. Used mainly for administration
	 * purposes.
	 */
	public void reset() {
	 ...
	 * 
	 * @return <code>true</code> if currently scanning iBeacons.
	...
	public static boolean configureBluetoothAdapter(Context c) {
		...
		final BluetoothManager bluetoothManager = (BluetoothManager) c
				.getSystemService(Context.BLUETOOTH_SERVICE);
		_ibp._bluetoothAdapter = bluetoothManager.getAdapter();
		if (_ibp._bluetoothAdapter == null
				|| !_ibp._bluetoothAdapter.isEnabled()) {
			return false;
		}
		return true;
	}
	...
	/**
	 * Callback for processing BLE events, to identify iBeacons during the
	 * scanning process.
	 */
	private BluetoothAdapter.LeScanCallback mLeScanCallback = new BluetoothAdapter.LeScanCallback() {
		...
		public void onLeScan(final BluetoothDevice device, int rssi,
			...
			// If already discovered, then just refresh the RSSI of the existing
			// instance and return
			if (_arrOrderedIBeacons.contains(newBeacon)) {
				...
				if (newDistance < oldDistance) {
					...
					// Sort again
					Collections.sort(_arrOrderedIBeacons,
							new IBeaconProximityComparator());
				}
				return;
			}
			...
			if (device.getName() != null) {
				if (device.getName().startsWith(EASIBEACON_IDPREFIX)) {
					...
					String version = device.getName().substring(
							EASIBEACON_IDPREFIX.length());
					...
					if (newBeacon.getVersion() == 1) {
						...
						if (!newBeacon.isConnectable())
							newBeacon.setEasiBeacon(false); // If not
															// connectable we
															// will report it as
															// unknown
					}
			...
			// Review this
			Log.i(Utils.LOG_TAG, device.getName() + " " + device.getAddress()
					+ " " + newBeacon.getPowerValue() + " " + rssi
					+ " Connectable: " + newBeacon.isConnectable());
			newBeacon.setProximity((int) calculateDistance(
					newBeacon.getPowerValue(), rssi));
			if (!_arrOrderedIBeacons.contains(newBeacon)) {
				...
				_timeoutHandler.postDelayed(timeoutTask,
	 ...
	 * @param b
	 *            ibeacon to find
	 * @return <code>null</code> if the ibeacon is not found, the existing copy
	 *         of the ibeacon otherwise.
	 */
	...
	/**
	 * Obtains a reference to the {@link android.bluetooth.BluetoothDevice}
	 * based on the MAC
	 * 
	 * @param mac
	 ...
	 * 
	 * @param uuid
	 *            the UUID to filter ibeacon advertisements
	 */
	 ...
	 * 
	 * @param enable
	 *            <code>true</code> to start scanning, <code>false</code> to
	...
	private IBeacon parseAdvertisementData(byte[] data) {
		...
		if (data[0] == 0x02 && data[1] == 0x01 && data[4] == (byte) 0xFF
				&& data[7] == 0x02) { // iBeacon candidate
			byte[] uuid = Arrays.copyOfRange(data, ADV_PREFIX_LENGTH,
					ADV_PREFIX_LENGTH + ADV_UUID_LENGTH);
			...
			if (_uuid == null || Arrays.equals(_uuid, uuid)) {
				...
				int major = ((data[offset] << 8) & 0x0000ff00)
						| (data[offset + 1] & 0x000000ff);
				ibeacon.setMajor(major);
				int minor = ((data[offset + 2] << 8) & 0x0000ff00)
	 ...
	 * @param data
	 *            advertisement data
	 * @return <code>true</code> if the easiBeacon is in connectable mode,
	 *         <code>false</code> otherwise.
	 */
	...
	/**
	 * Roughly estimates the distance to the iBeacon Calculation obtained from
	 * http
	 * ://stackoverflow.com/questions/20416218/understanding-ibeacon-distancing
	 * 
	 * @param txPower
	 *            RSSI of the iBeacon at 1 meter
	 * @param rssi
	 *            measured RSSI by the user device
	 * @return
	 */
