...
public class PromotionListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.textPromo = ((TextView) convertView
					.findViewById(R.id.offer_name));
			holder.imagePromo = (ImageView) convertView
					.findViewById(R.id.image_promo);
...
//		OfferDetailsDTO promo = EasiActivity.promotions.get(position);
		 OfferDetailsDTO promo = PromocionesFragment.promotions.get(position);
		holder.addBtn.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View v) {
				Toast toast = Toast.makeText(context,
						"Promocion guardado en tu Wallet", Toast.LENGTH_SHORT);
				toast.show();
		...
		holder.imagePromo.setImageDrawable(context.getResources().getDrawable(
				R.drawable.logo_smt));

		if (promo.getOfferType() == 2) {
			((BeaconsApp) context.getApplicationContext()).getImageLoader()
					.displayImage(promo.getOfferURL(), holder.imagePromo,
